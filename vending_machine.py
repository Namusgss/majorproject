...
# Sample product data
products = [
    {"name": "Drinks", "price": 100, "image": "images/coke.jpeg", "quantity": 1},
    {"name": "Noodles", "price": 150, "image": "images/waiwai.jpeg", "quantity": 1}
]
...
def update_quantity(product, change):
    ...
    # Update the quantity label text
...
def on_product_click(product):
    ...
    quantity_label.config(text=f"Quantity: {product['quantity']}")
    price_label.config(text=f"Price: ${product['price']}")
...
# Frame to hold product images (buttons) and names with prices
...
increase_button = tk.Button(root, text="+", command=lambda: update_quantity(products[0], 1), font=("Arial", 16), bg="#4CAF50", fg="white", relief="solid", width=5)
decrease_button = tk.Button(root, text="-", command=lambda: update_quantity(products[0], -1), font=("Arial", 16), bg="#4CAF50", fg="white", relief="solid", width=5)

# "Buy Now" button to generate the QR code for the selected product
buy_now_button = tk.Button(root, text="Buy Now", command=lambda: buy_now(products[0]), font=("Arial", 16), bg="#FF5722", fg="white", relief="solid", width=10)
...
qr_code_label = tk.Label(root, bg="#f2f2f2")
qr_code_label.pack_forget()  # Hide initially

# Loop through products and display images with names and prices
for product in products:
    ...
    if os.path.exists(product["image"]):
        ...
        # Create a button for each product image along with name and price
